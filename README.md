...
当前为 V2 Foundation 0.4.2：导航和页面已重构为“基础、听抄、发报、工具”四个功能域，统一训练路由、V2 本地数据、自由拍发、Morse 双向转换、字符速查、进度与设置均已接通。本轮完成全项目稳定性审计，修复离线旧路由、自由拍发待提交状态、暂停输入、训练快照音频参数、设置/导入校验与键盘焦点可见性。侧栏已移除装饰性编号，并完成 320–667px 响应式稳定化。V2 不读取旧会话、结果、课程进度或设置，旧 URL 不提供重定向。进入 Capacitor/Tauri 封装前仍需完成跨浏览器与真机矩阵。
