...
当前完成到 Stage C.1 稳定化版本：四种基础训练模式、可刷新恢复的会话、真实错题重练、字符筛选、持久化设置、数据导入导出、统计子页及 PWA 安装/离线/更新机制均已接通。进入 Capacitor/Tauri 封装前仍需完成跨浏览器与真机矩阵。
...
`npm test` 会执行生产构建、28 项领域/存储/路由/PWA 测试和服务端直达渲染检查。
