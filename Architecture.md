...
| 当前实现 | Stage C Web/PWA；稳定路由、领域引擎、IndexedDB 与离线应用壳已接通 |
...
- IA 中 P0 核心页面的稳定 URL、直达渲染与 History API 导航。
- 四种主题的设计令牌覆盖能力。
- Web Audio 的用户解锁、实时发声和定时播放。
- 单键长短按、双键点划、失焦停音和自动解码。
- 首次引导、可配置练习、逐题保存、刷新恢复与真实结果统计。
- Manifest、Service Worker 应用壳缓存、离线回退和用户确认更新。

### 3.2 尚未完成的正式产品能力

- IndexedDB v1 schema 和事务仓储已建立，设置统一存储、导入导出尚未实现。
- Service Worker 已实现核心页面离线启动，完整离线训练仍需在安装后的物理设备验证。
- 字符学习页暂只直接展示 36 个字母/数字，标点筛选尚未完成。
- 页面视图仍集中在 `app/_components/morse-app.tsx`，后续按功能边界继续拆分复用组件。
- Safari、Firefox、Android/iOS 真机音频与后台生命周期尚未验证。
...
- 当前 Web 客户端使用 Vinext 可选捕获路由承接直达请求，并以轻量 History API 适配器管理 `InformationArchitecture.md` 中的稳定 URL；后续页面拆包时保持同一路由协议。
...
- 当前使用静态 Manifest 和受测试的原生 Service Worker，避免与 Vinext 多环境构建产生插件耦合。
- 应用壳、Manifest、图标及核心入口进入预缓存；其余同源静态资源运行时缓存。
- 用户训练数据只存在 IndexedDB，不进入 Cache Storage。
- MVP 采用“提示更新”而不是会话中自动刷新。
- 有进行中会话时，新 Service Worker 不接管并触发页面重载。
- 每次应用壳发布必须同步递增 `public/sw.js` 的 `CACHE_VERSION`，以触发 waiting worker 更新流程。
...
**状态：已完成本地开发与 Chromium 核心验证（2026-07-15）。**

- [x] 建立稳定路由、直达渲染和响应式页面壳。
- [x] 迁移已验证的学习、练习、统计、设置和发报交互。
- [x] 实现 PWA Manifest、应用壳缓存、离线回退和更新策略。
- [x] 完成核心训练链路、刷新恢复、移动布局和语义检查。
- [ ] 完成 Safari、Firefox 及 Android/iOS 安装后离线真机矩阵。
